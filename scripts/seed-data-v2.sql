...
) AS v(name, type, quantity, price)
WHERE NOT EXISTS (
  SELECT 1 FROM products p WHERE p.name = v.name
);

-- Insert sample transactions (only if they don't exist)
INSERT INTO transactions (date, item_purchased, customer_name, store_name, payment_method, purchase_price, selling_price, revenue, notes, month, year)
SELECT * FROM (VALUES
...
) AS v(date, item_purchased, customer_name, store_name, payment_method, purchase_price, selling_price, revenue, notes, month, year)
WHERE NOT EXISTS (
  SELECT 1 FROM transactions t 
  WHERE t.item_purchased = v.item_purchased 
  AND t.customer_name = v.customer_name 
